    ...
    execute: async function(interaction, errEmb) {
        ...
        if(userGuild != null) {
            ...
            try {
                ...
                await user.send({ embeds: [emb.generate('Suite à votre demande de debug', null, `La demande de debug de la foncitonnalité **${interaction.message.embeds[0].title.split(': ')[1]}** à bien été revue mais n'a pas été jugée comme étant un bug ou le bug n'a pas pu être reproduit\n\nMerci tout même de votre report, c'est grâce aux personnes comme vous que l'on peut améliorer l'expérience utilisateur 💙`, '#FF0000', `https://cdn.discordapp.com/attachments/1083724872045297734/1084447275121647626/no.webp`, null, null, null, null, `Cordialement, ` + interaction.user.username, interaction.user.avatarURL(), true)] });
                await interaction.followUp({ embeds: [emb.generate('Demande annulée', null, `**${interaction.message.embeds[0].author.name}** à bien été prévenu du retour concernant sa demande !`, '#0DE600', process.env.LSMS_LOGO_V2, null, null, null, null, name, interaction.client.user.avatarURL(), true)], ephemeral: true });
