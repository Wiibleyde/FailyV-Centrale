        ...
        try {
            if(interaction.member.roles.cache.has(serviceID)) {
                ...
                if(interaction.member.roles.cache.has(dispatchID)) {
                    ...
                    embed = emb.generate(`Dispatch relâché`, null, `Vous avez relâché votre rôle de dispatcheur\n*À bientôt !*`, `#FFF1D0`, process.env.LSMS_LOGO_V2, null, `Gestion du service`, `https://cdn.discordapp.com/icons/${process.env.IRIS_PRIVATE_GUILD_ID}/${interaction.guild.icon}.webp`, null, null, null, false);
                    await logRP.fdd(interaction.guild, interaction.member.nickname, null);
                } else {
                    interaction.member.roles.add(switchRole);
                    embed = emb.generate(`Prise de dispatch`, null, `Vous êtes maintenant un dispatcheur\n*Bon courage pour votre gestion !*`, `#FFF1D0`, process.env.LSMS_LOGO_V2, null, `Gestion du service`, `https://cdn.discordapp.com/icons/${process.env.IRIS_PRIVATE_GUILD_ID}/${interaction.guild.icon}.webp`, null, null, null, false);
                    await logRP.pdd(interaction.guild, interaction.member.nickname, null);
                }
