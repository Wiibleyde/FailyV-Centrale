        ...
        try {
            if(interaction.member.roles.cache.has(serviceID)) {
                ...
                if(interaction.member.roles.cache.has(offID)) {
                    ...
                    embed = emb.generate(`Fin de off radio`, null, `Vous n'êtes plus en off radio\n*C'est reparti à fond les bananes !*`, `#000001`, process.env.LSMS_LOGO_V2, null, `Gestion du service`, `https://cdn.discordapp.com/icons/${process.env.IRIS_PRIVATE_GUILD_ID}/${interaction.guild.icon}.webp`, null, null, null, false);
                } else {
                    interaction.member.roles.add(switchRole);
                    embed = emb.generate(`Début de off radio`, null, `Vous êtes désormais en off radio`, `#000001`, process.env.LSMS_LOGO_V2, null, `Gestion du service`, `https://cdn.discordapp.com/icons/${process.env.IRIS_PRIVATE_GUILD_ID}/${interaction.guild.icon}.webp`, null, null, null, false)
                }
