...
module.exports = {
    ...
    //Action lors de l'execution de la commande
        ...
        const commandToDebug = new TextInputBuilder().setCustomId(`commandToDebug`).setLabel(`Nom de la commande/fonctionnalité buggé(e)`).setStyle(TextInputStyle.Short).setPlaceholder(`Ex: /lit`).setRequired(true);
        const problem = new TextInputBuilder().setCustomId(`problem`).setLabel(`Que fait-il et comment cela se produit-il ?`).setStyle(TextInputStyle.Paragraph).setPlaceholder(`Ex: Quand je l'exécute tout les boutons ne se génèrent pas`).setRequired(true);
        ...
        const screenshot = new TextInputBuilder().setCustomId(`screenshot`).setLabel(`URL d'un screen du bug`).setValue(screenURL).setStyle(TextInputStyle.Short).setPlaceholder(`Laisser vide si aucune image`).setRequired(false);
