...
module.exports = {
    ...
    async execute(interaction) {
        ...
        //Si le type est général
            ...
            if (IRIS_PSYCHO_CHANNEL_ID[0] == undefined) {
                psychoChannelId = null;
                const embed = emb.generate("Désolé :(", null, `Aucun channel psy n'a été trouvé dans la base de donnée, veuillez faire un </define:1140478983536918600> pour choisir un channel.`, "#FF0000", process.env.LSMS_LOGO_V2, null, `Gestion des rendez-vous`, `https://cdn.discordapp.com/icons/${process.env.IRIS_PRIVATE_GUILD_ID}/${interaction.guild.icon}.webp`, null, null, null, false);
                return await interaction.reply({ embeds: [embed], ephemeral: true });
            } else {
                psychoChannelId = interaction.guild.channels.cache.get(IRIS_PSYCHO_CHANNEL_ID[0].id);
            }
            ...
            if (IRIS_SURGERY_CHANNEL_ID[0] == undefined) {
                surgeryChannelId = null;
                const embed = emb.generate("Désolé :(", null, `Aucun channel chir n'a été trouvé dans la base de donnée, veuillez faire un </define:1140478983536918600> pour choisir un channel.`, "#FF0000", process.env.LSMS_LOGO_V2, null, `Gestion des rendez-vous`, `https://cdn.discordapp.com/icons/${process.env.IRIS_PRIVATE_GUILD_ID}/${interaction.guild.icon}.webp`, null, null, null, false);
                return await interaction.reply({ embeds: [embed], ephemeral: true });
            } else {
                surgeryChannelId = interaction.guild.channels.cache.get(IRIS_SURGERY_CHANNEL_ID[0].id);
            }
            ...
            if (IRIS_GENERAL_CHANNEL_ID[0] == undefined) {
                generalChannelId = null;
                const embed = emb.generate("Désolé :(", null, `Aucun channel général n'a été trouvé dans la base de donnée, veuillez faire un </define:1140478983536918600> pour choisir un channel.`, "#FF0000", process.env.LSMS_LOGO_V2, null, `Gestion des rendez-vous`, `https://cdn.discordapp.com/icons/${process.env.IRIS_PRIVATE_GUILD_ID}/${interaction.guild.icon}.webp`, null, null, null, false);
                return await interaction.reply({ embeds: [embed], ephemeral: true });
            } else {
                generalChannelId = interaction.guild.channels.cache.get(IRIS_GENERAL_CHANNEL_ID[0].id);
            }
