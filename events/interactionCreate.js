...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if(interaction.isChatInputCommand()) {
            ...
            await logger.log(`${interaction.member.nickname} - ${interaction.user.username}#${interaction.user.discriminator} (${interaction.user})\n\na utilisé(e) la commande "/${interaction.commandName}"`);
            const command = interaction.client.commands.get(interaction.commandName);
    
            if(!command) { await logger.error(`Aucune commande correspondante à ${interaction.commandName} n'a été trouvée !`); return; }
