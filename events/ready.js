...
module.exports = {
    ...
    async execute(client) {
        ...
        if(process.env.MODE == 'DEV') { client.user.setPresence({ activities: [{ name: `🤖 Dev mode!`, type: ActivityType.Competing }], status: 'idle' }); }
            ...
            let isBlackout = service.isBlackout();
            let state = 'online';
            if(isDebugMode) { state = 'dnd'; }
            if(isBlackout) { state = 'invisible' }
            client.user.setPresence({ activities: [{ name: `🚑 ` + serviceCount + ` | 🎙️ ` + dispatchCount, type: ActivityType.Watching }], status: state });
            setInterval(async () => {
                isBlackout = service.isBlackout();
        ...
        const update = new CronJob('00 55 05 * * *', async function() {
            ...
            await wait(30000);
            logger.log(`Redémarrage dans **30s** pour la MAJ du jour !`);
            await wait(30000);
        ...
        const updateOrgans = new CronJob('00 00 00 * * *', async function() {
            const sqlConfig = require('../sql/config/config');
            const organsChannelId = await sqlConfig.getChannel('follow');
            if(organsChannelId[0] != null) {
