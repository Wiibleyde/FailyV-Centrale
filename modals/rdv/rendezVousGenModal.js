...
module.exports = {
    ...
    execute: async function (interaction, errEmb) {
        ...
        //Create embed
        const rendezVousEmb = emb.generate(null, null, null, process.env.LSMS_COLORCODE, process.env.LSMS_LOGO_V2, null, `Prise de rendez-vous`, `https://cdn.discordapp.com/icons/${process.env.IRIS_PRIVATE_GUILD_ID}/${interaction.client.guilds.cache.get(process.env.IRIS_PRIVATE_GUILD_ID).icon}.webp`, null, pseudo, null, false);
        ...
        const channelToSend = interaction.client.guilds.cache.get(process.env.IRIS_PRIVATE_GUILD_ID).channels.cache.get(process.env.IRIS_GENERAL_CHANNEL_ID);
        ...
        const rendezVousActionRow = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId('rendezVousFini').setLabel("Terminer/Supprimer").setStyle(ButtonStyle.Primary).setEmoji("896393106700775544").setDisabled(false),
            new ButtonBuilder().setCustomId('rendezVousContacte').setLabel("Personne contactée").setStyle(ButtonStyle.Danger).setEmoji("📞").setDisabled(false),
            new ButtonBuilder().setCustomId('rendezVousPris').setLabel("Rendez-vous pris").setStyle(ButtonStyle.Secondary).setEmoji("📆").setDisabled(false)
        );
        //Save RDV in DB
        await rdv.registerRDV(interaction.user.id, phoneNumber, "Généraliste", interaction.components[2].components[0].value);
        //Send embed with buttons
        channelToSend.send({ embeds: [rendezVousEmb], components: [rendezVousActionRow] });
        //Send confirmation message
        await interaction.reply({ embeds: [emb.generate(null, null, `Le rendez-vous a bien été ajouté à l'agenda !`, `#0DE600`, process.env.LSMS_LOGO_V2, null, `Prise de rendez-vous`, `https://cdn.discordapp.com/icons/${process.env.IRIS_PRIVATE_GUILD_ID}/${interaction.client.guilds.cache.get(process.env.IRIS_PRIVATE_GUILD_ID).icon}.webp`, null, null, null, false)], ephemeral: true });            // Supprime la réponse après 5s
        await wait(5000);
