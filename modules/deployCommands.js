...
module.exports = {
    init: (client) => {
            ...
            if(folder == 'test') {
                ...
                for(const file of commandFiles) {
                    ...
                    if('data' in command && 'execute' in command) {
                        ...
                        await logger.warn(`La commande ${filePath} n'a pas la partie "data" ou "execute" !`);
                    }
                }
        ...
        (async () => {
            try {
                await logger.log(`Rechargement de ${debugCommands.length} commandes slash (/) de dev.`);
                await logger.log(`Rechargement de ${privateCommands.length} commandes slash (/) privées.`);
                await logger.log(`Rechargement de ${publicCommands.length} commandes slash (/) publiques.`);
                ...
                await logger.log(`Rechargement de ${privateData.length + publicData.length + debugData.length} commandes slash (/) réussies.`);
            } catch (error) {
